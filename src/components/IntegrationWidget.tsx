...
export const IntegrationWidget: React.FC<IntegrationWidgetProps> = ({
  ...
  const [savedWidgetJson, setSavedWidgetJson] = useState<string>(() => JSON.stringify(widgetSettings));

  React.useEffect(() => {
    setSavedWidgetJson(JSON.stringify(widgetSettings));
  }, []);

  const isWidgetDirty = React.useMemo(() => {
    return JSON.stringify(widgetSettings) !== savedWidgetJson;
  }, [widgetSettings, savedWidgetJson]);

  const [isSavingWidget, setIsSavingWidget] = useState(false);
  const [widgetSaveStatus, setWidgetSaveStatus] = useState<{
    type: 'success' | 'error';
    message: string;
  } | null>(null);
  ...
  const handleSaveWidgetSettings = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errData.error || `Phản hồi lỗi mã ${res.status}`);
      }

      setSavedWidgetJson(JSON.stringify(widgetSettings));
      setWidgetSaveStatus({
        type: 'success',
        message: '🎉 Đã lưu tùy chỉnh Widget thành công!'
      });
    } catch (e: any) {
      ...
      setWidgetSaveStatus({
        type: 'error',
        message: `⚠️ Đã lưu tại trình duyệt, nhưng gặp lỗi kết nối với máy chủ: ${e?.message || 'Không có phản hồi'}`
      });
    } finally {
      setIsSavingWidget(false);
      setTimeout(() => setWidgetSaveStatus(null), 5000);
    }
  };
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-2xs space-y-4">
            ...
            <div className="space-y-4 text-xs">
              ...
              {widgetSaveStatus && (
                <div className={`p-3 rounded-xl border text-xs font-semibold flex items-center justify-between gap-2 animate-fadeIn ${
                  widgetSaveStatus.type === 'success'
                    ? 'bg-emerald-50 border-emerald-200 text-emerald-900'
                    : 'bg-rose-50 border-rose-200 text-rose-900'
                }`}>
                  <span className="flex items-center gap-1.5">
                    {widgetSaveStatus.type === 'success' ? '🎉' : '⚠️'} {widgetSaveStatus.message}
                  </span>
                  <button type="button" onClick={() => setWidgetSaveStatus(null)} className="text-slate-400 hover:text-slate-600 font-bold px-1.5 cursor-pointer">✕</button>
                </div>
              )}
              ...
              <div className="pt-3 border-t border-slate-100 flex items-center justify-between gap-3">
                <div className="flex-1 text-xs">
                  {isWidgetDirty ? (
                    <span className="text-amber-600 font-semibold flex items-center gap-1.5 animate-pulse">
                      ...
                      Có thông tin Widget mới chưa lưu
                    </span>
                  ) : (
                    <span className="text-slate-500">
                      Tùy chỉnh Widget đã được lưu
                    </span>
                  )}
                </div>
                <button
                  ...
                  className={`px-5 py-2.5 rounded-xl text-xs font-bold transition-all shadow-sm flex items-center gap-2 cursor-pointer disabled:opacity-50 ${
                    isWidgetDirty
                ...
                >
                  <Save className={`w-4 h-4 ${isWidgetDirty ? 'text-white' : 'text-slate-300'}`} />
                  <span>
                    {isSavingWidget
                      ? 'Đang lưu Widget...'
                      : isWidgetDirty
                      ? 'Lưu Tùy Chỉnh Widget (Có thay đổi)'
                      : 'Lưu Tùy Chỉnh Widget'}
                  </span>
                </button>
              </div>
            </div>
